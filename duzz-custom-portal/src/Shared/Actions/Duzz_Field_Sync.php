...
class Duzz_Field_Sync {
    ...
    function __construct() {
        ...
        add_action('acf/save_post', [$this, 'sync_customer_name_and_address_fields'], 20);
        add_action('save_post', [$this, 'sync_customer_name_and_address_fields'], 20);
        add_action('duzz_fields_updated', [$this, 'sync_customer_name_and_address_fields'], 20);
        add_filter('acf/prepare_field', [$this, 'make_individual_name_and_address_fields_read_only']);
    }

    function sync_customer_name_and_address_fields($post_id) {
        ...
        $formatted_website = Duzz_Format_Label::format_website($website);
        $formatted_email = Duzz_Format_Label::format_website($customer_email);  
        ...
        if (is_string($customer_name)) {
               $formatted_names = Duzz_Format_Label::format_user_name($customer_name);
        ...
        $formatted_address = Duzz_Format_Label::format_address($address1, $address2, $city, $state, $zip);      
    ...
    function make_individual_name_and_address_fields_read_only($field) {


        $read_only_fields = [
            Duzz_Keys::get_acf_key('customer_first_name'),
            Duzz_Keys::get_acf_key('customer_last_name'),
            Duzz_Keys::get_acf_key('customer_address'),
        ];
