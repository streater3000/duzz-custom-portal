...
$lockedColumns = array(
   ...
   $hook_column = array(); // Initialize $hook_column array to avoid null value

    // Hook to add additional columns
    $hook_column = apply_filters('duzz_additional_columns', $hook_column);

    // If $hook_column is not an array, set it to an empty array
    if (!is_array($hook_column)) {
        $hook_column = array();
    }

    // Merge $selected_columns, $hook_column, and $lockedColumns
    $all_columns = array_merge($selected_columns, $hook_column, $lockedColumns);
    ...
    foreach ( $posts as $post ) {
        ...
        list($new_post, $new_post_title) = $postUpdateInstance->duzz_updateProject($post_id);
            ...
            foreach ( $all_columns as $column_name => $field_key ) {
                ...
                if ($column_name === 'status_update') {
                    ...
                    } elseif (in_array($field_key, $hook_column)) {
                    // Apply filter to modify 'subproject_id' values
                    $post_data[$column_name] = apply_filters('duzz_custom_column_data', null, $post_id);
                } else {
        ...
        $table = new Duzz_Table_Factory($this->post_type, $this->include_fields, $data_title_string, $this->post_id);
