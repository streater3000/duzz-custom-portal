...
public function tribute() {
    $project_id = get_query_var('project_id', false); // Retrieving 'project_id' from URL
    
    if (!$project_id) return ''; // If project_id is not set, return empty string.
    ...
    public function duzz_send_project_tagged_notification($id, $comment) {
        ...
        foreach ($emails as $email) {
            ...
            $url_project_link = site_url('/your-project/' . $post_id . '/');
...
public function duzz_send_project_update_notification($id, $comment) {
    ...
    // Get the project id from query var
    $project_id = get_query_var('project_id', 0); // The second parameter is the default value if 'project_id' is not set
    ...
    foreach ($users as $user) {
        ...
        $data = [
            ...
            'content' => $comment->comment_content,
            'project_url' => site_url('/view-project/' . $post_id), // Updated to use the new format
            'project_id' => $post_id,
        ];
